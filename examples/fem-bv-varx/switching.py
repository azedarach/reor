"""
Run FEM-BV-VARX on autoregressive system with known switching process.
"""
...
def generate_data(n_features=2, n_samples=500, n_components=2, n_switches=3,
                  order=1, noise_variance=1.0, random_state=None):
    ...
    mu = rng.normal(size=(n_components, n_features,))

    if order > 0:
        A = rng.normal(size=(n_components, order, n_features, n_features))
    else:
        A = None

    if n_switches == 0:
        max_residence_length = n_samples
    else:
        max_residence_length = int(np.floor(n_samples / (n_switches + 1)))

    initial_component = rng.choice(np.arange(n_components))

    true_weights = np.empty((n_samples, n_components))
    for i in range(order):
        true_weights[i, initial_component] = 1

    component = initial_component
    residence_length = order
    for t in range(order, n_samples):
        true_weights[t, component] = 1

        xt = mu[component]
        if order > 0:
            for m in range(1, order + 1):
                xt += np.dot(A[component, m - 1], X[t - m])
        ...
        if residence_length >= max_residence_length:
            residence_length = 0
            component = (component + 1) % n_components

    return X, mu, A, true_weights
...
def plot_weights_timeseries(weights, output_file=None):
...
def parse_cmd_line_args():
    ...
    parser.add_argument('--true-n-components', dest='true_n_components', type=int,
                        default=2, help='number of components')
    parser.add_argument('--n-switches', dest='n_switches', type=int, default=3,
                        help='period of switches in true weights')
...
def main():
    ...
    X, mu, A, true_weights = generate_data(
        args.n_features, n_samples=args.n_samples,
        n_components=args.true_n_components, n_switches=args.n_switches,
        order=args.order, noise_variance=args.noise_variance,
    ...
    if args.plot_weights:
        plot_weights_timeseries(best_weights)
