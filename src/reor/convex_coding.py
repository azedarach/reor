...
def _convex_coding_log_likelihood_fn(data, mu, w, z, sigma_sq, concentration):
    ...
    print(z)
    z_safe = tf.math.maximum(z, np.finfo(z.dtype.as_numpy_dtype).tiny)

    log_likelihood += tf.reduce_sum(z_dist.log_prob(z_safe))

    std_dev = tf.dtypes.cast(tf.sqrt(sigma_sq), data.dtype)
    x_dist = tfp.distributions.Normal(
        loc=tf.dtypes.cast(mu[tf.newaxis, :] + tf.matmul(z, w), data.dtype),
        scale=std_dev * tf.ones(data.shape, dtype=data.dtype))
...
def _convex_coding_log_joint_fn(data, mu, w, z, sigma_sq, concentration,
    ...
    if sigma_w > 0:
        w_dist = tfp.distributions.Normal(
            loc=tf.zeros([n_components, n_features], dtype=w.dtype),
            scale=sigma_w * tf.ones([n_components, n_features], dtype=w.dtype))

        log_joint += tf.reduce_sum(w_dist.log_prob(w))

    # Log-likelihood contribution
    log_joint += _convex_coding_log_likelihood_fn(
        data, mu, w, z, sigma_sq, concentration)
...
def convex_coding_map_estimate(data, n_components, concentration,
    ...
    w_init : None or Tensor, shape (n_components, n_features)
        If a Tensor, used as initial guess for solution.

    z_init : None or Tensor, shape (n_samples, n_components)
        If a Tensor, used as initial guess for solution.
    ...
    w_map : Tensor, shape (n_components, n_features)
        MAP estimate for the latent features.

    z_map : Tensor, shape (n_samples, n_components)
    ...
    if w_init is None:
        w_map = tf.Variable(
            tf.random.normal([n_components, n_features],
                             dtype=data.dtype))
    else:
    ...
    if z_init is None:
        z_vals = tf.random.uniform([n_samples, n_components],
                                   dtype=data.dtype)
        z_vals /= tf.reduce_sum(z_vals, axis=1, keepdims=True)
    else:
        z_vals = tf.constant(z_init, dtype=data.dtype)

    # Note constrained latent variables are transformed to unconstrained
    # space
    transform = tfp.bijectors.SoftmaxCentered()
    a_map = tf.Variable(transform.inverse(z_vals), dtype=data.dtype)
    ...
    for epoch in range(max_epochs):
        ...
        def target(mu, w, a, sigma_sq):
            z = transform.forward(a)
            return -_convex_coding_log_joint_fn(
                data, mu, w, z, sigma_sq, concentration,
                sigma_mu=sigma_mu, sigma_w=sigma_w, beta=beta)

        with tf.GradientTape() as tape:
            loss_value = target(mu_map, w_map, a_map, sigma_sq_map)

        grads = tape.gradient(loss_value, [mu_map, w_map, a_map, sigma_sq_map])
        print(grads[2])
        optimizer.apply_gradients(
            zip(grads, [mu_map, w_map, a_map, sigma_sq_map]))
    ...
    return mu_map, w_map, transform.forward(a_map), sigma_sq_map, loss_value
...
class ConvexCoding():
    ...
    def fit_transform(self, data, **kwargs):
        ...
        Returns
        -------
        z : Tensor, shape (n_samples, n_components)
    ...
    def transform(self, data, optimizer=None, max_epochs=200, tolerance=-1):
        ...
        z_vals = tf.random.uniform([n_samples, self.n_components],
                                   dtype=data.dtype)
        z_vals /= tf.reduce_sum(z_vals, axis=0, keepdims=True)

        transform = tfp.bijectors.SoftmaxCentered()
        a_map = tf.Variable(transform.inverse(z_vals))
        ...
        def target(a):
            z = transform.forward(a)
            return -_convex_coding_log_joint_fn(
                data, self.mu, self.w, z, self.sigma_sq, self.concentration,
        ...
        for epoch in range(max_epochs):
            ...
            with tf.GradientTape() as tape:
                loss_value = target(a_map)

            grads = tape.gradient(loss_value, [a_map])

            optimizer.apply_gradients(zip(grads, [a_map]))
        ...
        return transform.forward(a_map)
    ...
    def inverse_transform(self, z):
        ...
        Returns
        -------
        x_new : Tensor, shape (n_samples, n_features)
            Latent variables transformed to original space.
        """

        return self.mu[tf.newaxis, :] + tf.matmul(z, self.w)
